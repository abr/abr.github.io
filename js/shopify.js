...
const ShopifyBuyInit = () => {
  ...
  ShopifyBuy.UI.onReady(client).then(function (ui) {
    const nodes = document.getElementsByClassName('shopify-collection');
    for (let i = 0; i < nodes.length; i++) {
      ui.createComponent('collection', {
        id: nodes[i].dataset.collectionid,
        node: nodes[i],
...
const OPTIONS = {
  "product": {
    "styles": {
      ...
      "button": {
        "box-shadow": "inset 0 -1px 0 0 rgba(0,0,0,0.15), inset 0 1px 0 0 rgba(255,255,255,0.25)",
  ...
  "cart": {
    "templates" :{
      ...
      <div class="{{data.classes.cart.footer}}" data-element="cart.footer">
        ...
        <button class="{{data.classes.cart.button}}" id="checkout" type="button" data-element="cart.button" disabled>{{data.text.button}}</button>
      </div>
